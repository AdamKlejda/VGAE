...
def parseArguments():
    ...
    parser.add_argument('-genformat', required=False, default="0",
    ...
    parser.add_argument("-popsize", type=int, default=10, help="Population size, default 100.")
    parser.add_argument('-generations', type=int, default=20, help="Number of generations, default 2k.")
    parser.add_argument('-tournament', type=int, default=3, help="Tournament size, default 3.")
    parser.add_argument('-path_out', type=str, required=True, help="Path where to save hof and logs")
...
def main():
    ...
    new_generation_steps = [
        FramsCrossAndMutate(frams_lib, cross_prob=0.0, mutate_prob=1.0),
        fitness_remove
    ]
    ...
    end_steps = [PopulationSave(parsed_args.path_out+str(parsed_args.id)+"/"+"halloffame.gen", provider=hall_of_fame.halloffame,
    ...
    if os.path.exists(parsed_args.path_out+str(parsed_args.id)+"/"):
        print(parsed_args.path_out+str(parsed_args.id)+"/","exists")  
    else:
        os.makedirs(parsed_args.path_out+str(parsed_args.id)+"/")
    experiment.init()
    experiment.stats_log=stats_deap.logbook
    experiment.run(parsed_args.generations)
    
    log_book_df = pd.DataFrame(stats_deap.logbook)
    log_book_df.to_csv(parsed_args.path_out+str(parsed_args.id)+"/"+"logs.csv",index=False)
